...
if [[ "$MONITOR" != "LAPTOP" ]] && [[ "$MONITOR" != "TOP" ]] && [[ "$MONITOR" != "PRESENTATION" ]] && [[ "$MONITOR" != "RIGHT" ]] && [[ "$MONITOR" != "STANDALONE" ]] ; then
...
ln -sf "$GIT_PATH/linux-setup/miscellaneous/arandr_scripts/yurii/$MONITOR_LOWERCASE".sh ~/.monitor.sh
