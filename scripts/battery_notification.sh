...
send_notification() {
    ...
    local message="$2"
    local icon="$3"
    /usr/bin/dunstify -t 6000 -u critical -i "$icon" "$title" "$message"
}

# Check battery status and send notifications
if [[ "$BATTERY_PERCENTAGE" -ge $CHARGED_THRESHOLD ]] && [[ $AC_POWER -eq 1 ]]; then
    send_notification "Battery Fully Charged" "Your battery is fully charged." "battery-full" && mpg123 \
        "$GIT_PATH"/linux-setup/miscellaneous/notifications/notificacioncool.mp3
elif [[ "$BATTERY_PERCENTAGE" -le $LOW_THRESHOLD ]] && [[ $AC_POWER -eq 0 ]]; then
    send_notification "Low Battery" "Your battery is below $LOW_THRESHOLD%. Please connect charger." "battery-caution" && mpg123 \
        "$GIT_PATH"/linux-setup/miscellaneous/notifications/low_battery.mp3
elif [[ $BATTERY_PERCENTAGE -le $CRITICAL_THRESHOLD ]] && [[ $AC_POWER -eq 0 ]]; then
